...
I have added a slider that allows you to replace a number of characters (0-128) of the original string with their ALTCODE equivalents instead of a STRING command.  This really only works on Windows boxes, but should help confuse keyloggers. On linux sometimes it just drops the altcodes during transmission.  
**Special characters may be possible if the original string is completely rendered in altcodes with no introduced errors (the alt slider is full, the error slider is empty)
